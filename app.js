...
function card(a){const meta=[a.birthDate?`🎂 ${new Intl.DateTimeFormat("fr-FR").format(new Date(a.birthDate+"T00:00:00"))}`:"",a.location?`📍 ${esc(a.location)}`:"",a.origin?`📄 ${esc(a.origin)}`:""].filter(Boolean).join(" · ");return `<div class="card"><div class="animal"><div class="avatar">${icons[a.type]||"🐾"}</div><div><b>${esc(a.name||"Sans nom")}</b><div class="muted">${esc(a.species)} · ${esc(a.type)}</div></div></div>${meta?`<p class="muted">${meta}</p>`:""}<div style="display:flex;gap:8px;margin-top:10px"><button class="ghost animalAction" data-id="${a.id}" style="flex:1">Ajouter une activité</button><button class="ghost deleteAnimal" data-id="${a.id}" title="Supprimer cet animal">🗑️</button></div></div>`}
