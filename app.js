...
function card(a){return `<div class="card"><div class="animal"><div class="avatar">${icons[a.type]||"🐾"}</div><div><b>${esc(a.name)}</b><div class="muted">${esc(a.species)} · ${esc(a.type)}</div></div></div>${a.terrarium?`<p class="muted">🏠 ${esc(a.terrarium)}</p>`:""}<div style="display:flex;gap:8px;margin-top:10px"><button class="ghost animalAction" data-id="${a.id}" style="flex:1">Ajouter une activité</button><button class="ghost deleteAnimal" data-id="${a.id}" title="Supprimer cet animal">🗑️</button></div></div>`}
...
document.addEventListener("submit",e=>{if(e.target.id!=="animalForm")return;e.preventDefault();if(e.target.dataset.saving==="1")return;e.target.dataset.saving="1";const submit=e.target.querySelector('button[type="submit"]');if(submit)submit.disabled=true;const f=new FormData(e.target);const name=String(f.get("name")||"").trim();const species=String(f.get("species")||"").trim();if(!name||!species){e.target.dataset.saving="0";if(submit)submit.disabled=false;return;}db.animals.push({id:crypto.randomUUID(),name,species,type:f.get("type"),terrarium:String(f.get("terrarium")||"").trim(),notes:String(f.get("notes")||"").trim()});save();close()});
